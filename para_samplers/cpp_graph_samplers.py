...
from base_graph_samplers import (
    NodeIIDBase,
    KHopSamplingBase,
    PPRSamplingBase,
    Subgraph
...
try:
    ...
    import para_samplers.ParallelSampler as cpp_para_sampler
import numpy as np
import os
import glob
import sys
...
class GraphSamplerEnsemble:
    def __init__(
        ...
        config={'k': 200, 'threshold': 0.01, 'epsilon': '1e-5', 'add_self_edge': True, 'method': 'ppr', 'size_root': 1, 
        ...
        args_Cpp_sampler = [node_target, num_subg_per_batch, max_num_threads, config["fix_target"], config["sequential_traversal"], 1]
        args_Cpp_sampler = [adj.indptr, adj.indices, adj.data] + args_Cpp_sampler + ["", "", ""]
        
        print('args_Cpp_sampler:',args_Cpp_sampler)
        print('len(node_target):',len(node_target))
        self.para_sampler = cpp_para_sampler.ParallelSampler(*args_Cpp_sampler)
        
        sampler_config_list=[{'k': 200, 'threshold': 0.01, 'epsilon': '1e-5', 'add_self_edge': True, 'method': 'ppr', 'size_root': 1, 
        'fix_target': True, 'sequential_traversal': True, 'type_': 0, 'name_data': 'arxiv', 
        'dir_data': {'local': './data', 'is_adj_changed': False, 'is_feat_changed': False}, 'is_transductive': True, 'is_preproc': True}]


        self.sampler_list=[]
        sampler_config_list[0]["para_sampler"] = self.para_sampler      # share a common one
        self.sampler_list.append(PPRSampling(adj, node_target, {'hops'} **sampler_config_list[0]))
        
        
        self.cpp_config_list = [{'method': 'ppr', 'k': '200', 'num_roots': '1', 
                'threshold': '0.01', 'add_self_edge': 'true', 'return_target_only': 'false'}]
        self.cpp_aug_list = [{'hops'}]
    ...
    def par_sample_ensemble(self):
        ...
        ret[0] = self._extract_subgraph_return(ret[0], self.cpp_config_list[0], self.cpp_aug_list[0], True)
