...
def load_data(dataset, degree_as_tag):
    ...
    with open('/new_disk_B/scy/dataset2/%s/%s.txt' % (dataset, dataset), 'r') as f:
    ...
    with open('/new_disk_B/scy/dataset2/%s/%s.txt' % (dataset, dataset+'_motif2A'), 'r') as f:
        mat_list = []
        line = f.readline()
        while line:
            line = line.split(' ')
...
def load_hyper_data(dataset, degree_as_tag,LEN_TAGSET):
    '''
        dataset: name of dataset
        test_proportion: ratio of test train split
        seed: random seed for random splitting of dataset
    ...
    with open('/new_disk_B/scy/dataset2/%s/%s_motifs.txt' % (dataset, dataset), 'r') as f:
        n_g = int(f.readline().strip())
        for i in range(n_g):
            row = f.readline().strip().split()
            n, l = [int(w) for w in row]
    ...
    for g in g_list:
        ...
        if len(g.g)==0 or len(g.g)==1:
            ...
            deg_list = list(dict(g.g.degree(range(len(g.g)))).values())
            g.edge_mat = torch.LongTensor(edges).transpose(0,1)
            #print('edge mat:',g.edge_mat.shape)
...
def separate_data_val(graph_list,hyper_graph_list,motif2A_list, seed, fold_idx):
    assert 0 <= fold_idx and fold_idx < 10, "fold_idx must be from 0 to 9."
    skf = StratifiedKFold(n_splits=10, shuffle = True, random_state = seed)
    labels = [graph.label for graph in graph_list]
    idx_list = []
